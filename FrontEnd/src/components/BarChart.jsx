import { useTheme } from "@mui/material";
import { ResponsiveBar } from "@nivo/bar";
import { tokens } from "../theme";

const employeeData = [
  {
    "Employee ID": 1001,
    "Name": "Anbu",
    "Role": "Software Engineer",
    "Salary": 85000,
    "Performance Score": 4.5,
  },
  {
    "Employee ID": 1002,
    "Name": "Nithya",
    "Role": "Product Manager",
    "Salary": 75000,
    "Performance Score": 4.2,
  },
  {
    "Employee ID": 1003,
    "Name": "Dhivya",
    "Role": "Data Analyst",
    "Salary": 72000,
    "Performance Score": 3.8,
  },
  {
    "Employee ID": 1004,
    "Name": "Diana",
    "Role": "HR Specialist",
    "Salary": 65000,
    "Performance Score": 4,
  },
  {
    "Employee ID": 1005,
    "Name": "Viha",
    "Role": "DevOps Engineer",
    "Salary": 80000,
    "Performance Score": 4.7,
  },
];

const BarChart = ({ isDashboard = false }) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);

  // Transform data for the chart
  const transformedData = employeeData.map((employee) => ({
    country: employee.Name,
    salary: employee.Salary,
    performance: employee["Performance Score"],
  }));

  return (
    <ResponsiveBar
      data={transformedData}
      theme={{
        axis: {
          domain: {
            line: {
              stroke: colors.grey[100],
            },
          },
          legend: {
            text: {
              fill: colors.grey[100],
            },
          },
          ticks: {
            line: {
              stroke: colors.grey[100],
              strokeWidth: 1,
            },
            text: {
              fill: colors.grey[100],
            },
          },
        },
        legends: {
          text: {
            fill: colors.grey[100],
          },
        },
      }}
      keys={["salary", "performance"]}
      indexBy="country"
      margin={{ top: 50, right: 130, bottom: 50, left: 60 }}
      padding={0.3}
      valueScale={{ type: "linear" }}
      indexScale={{ type: "band", round: true }}
      colors={({ id }) => (id === "salary" ? "#976e99" : "#FFA500")}

      defs={[
        {
          id: "dots",
          type: "patternDots",
          background: "inherit",
          color: "#38bcb2",
          size: 4,
          padding: 1,
          stagger: true,
        },
        {
          id: "lines",
          type: "patternLines",
          background: "inherit",
          color: "#eed312",
          rotation: -45,
          lineWidth: 6,
          spacing: 10,
        },
      ]}
      borderColor={{
        from: "color",
        modifiers: [["darker", "1.6"]],
      }}
      axisTop={null}
      axisRight={null}
      axisBottom={{
        tickSize: 5,
        tickPadding: 5,
        tickRotation: 0,
        legend: isDashboard ? undefined : "Employee Name",
        legendPosition: "middle",
        legendOffset: 32,
      }}
      axisLeft={{
        tickSize: 5,
        tickPadding: 5,
        tickRotation: 0,
        legend: isDashboard ? undefined : "Value",
        legendPosition: "middle",
        legendOffset: -40,
      }}
      enableLabel={false}
      labelSkipWidth={12}
      labelSkipHeight={12}
      labelTextColor={{
        from: "color",
        modifiers: [["darker", 1.6]],
      }}
      legends={[
        {
          dataFrom: "keys",
          anchor: "bottom-right",
          direction: "column",
          justify: false,
          translateX: 120,
          translateY: 0,
          itemsSpacing: 2,
          itemWidth: 100,
          itemHeight: 20,
          itemDirection: "left-to-right",
          itemOpacity: 0.85,
          symbolSize: 20,
          effects: [
            {
              on: "hover",
              style: {
                itemOpacity: 1,
              },
            },
          ],
        },
      ]}
      role="application"
      barAriaLabel={function (e) {
        return e.id + ": " + e.formattedValue + " in employee: " + e.indexValue;
      }}
    />
  );
};

export default BarChart;
